...
def build_stacking_models(subsets, data_path):
    data = utils.loadData(data_path)
    n_columns = data.shape[1]
    x_instances = data[:, :n_columns - 1]
    y_instances = data[:, -1]

    source_folder = './stacking/models_and_evaluation'
    ...
    for i, subset in enumerate(subsets):
        number = 1 + i
        model_id.append('Stk-' + str(number))
        name_model_file = folders + '/Stk-' + str(number) + '.pickle'
        stacking = StackingClassifier(estimators=subset,
        ...
        for metrica in metricas:
            scoring = cross_val_score(stacking, x_instances, y_instances, scoring=metrica, cv=5)
            performance[i].append(round(scoring.mean(), 2))

        model = stacking.fit(x_instances, y_instances)
        joblib.dump(model, r'' + name_model_file)
    print("Modelos Stacking construidos")
    ...
    name_graph = source_folder + '/stacking_barplot.png'
    title = 'Evaluación del multiclasificador Stacking'
